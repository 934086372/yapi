...
export default class Project extends Component {
  ...
  render() {
    ...
    let routers = {
      interface: { name: '接口', path: '/project/:id/interface/api', component: Interface },     
      test: { name: '测试', path: '/project/:id/test/:action', component: Test },
      activity: { name: '动态', path: '/project/:id/activity', component: Activity },
      data: { name: '数据', path: '/project/:id/data', component: ProjectData },
      members: { name: '成员', path: '/project/:id/members', component: ProjectMember },
    ...
    Object.keys(routers).forEach(key => {
      ...
      if(key == 'test'){
         value = {
           name: item.name,
           path: `/project/${match.params.id}/test/col`
         };
      }else{
        value = {
          name: item.name,
    ...
    return (
      <div style={{display: 'flex', flexDirection: 'row', height: 'calc(100vh - 0.56rem)'}}>
        <Subnav default={defaultName} data={subnavData} style={{ padding: 0 , margin:0 }} />
        <Switch>
