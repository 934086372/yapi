  ...
  onSelect = keys => {
    if (keys.length) {
      ...
      if (type === 'col') {
        ...
        this.props.history.push('/project/' + project_id + '/test/col/' + id);
      } else {
        this.props.setColData({
          isRander: false
        });
        this.props.history.push('/project/' + project_id + '/test/case/' + id);
      }
    }
    this.setState({
      expands: null
    ...
    confirm({
      ...
      async onOk() {
        ...
        if (!res.data.errcode) {
          ...
          that.props.history.push('/project/' + params.id + '/test/col/' + nextColId);
        } else {
          message.error(res.data.errmsg);
        }
      }
  ...
  caseCopy = async caseId=> {
    ...
    const res = await axios.post('/api/col/add_case',data);
      if (!res.data.errcode) {
        ...
        await this.getList();
        this.props.history.push('/project/' + projectId + '/test/col/' + colId);
        this.setState({
          visible: false
        });
      } else {
      ...
      async onOk() {
        ...
        if (!res.data.errcode) {
          ...
          if (+caseId === +that.props.currCaseId) {
            that.props.history.push('/project/' + params.id + '/test/col/');
          } else {
