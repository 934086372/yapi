...
class InterfaceEditForm extends Component {
  ...
  // 处理 返回示例 desc
  handleResExample = d => {
    const initExample = this.state.desc;
    this.setState({
      desc: d.text
    });
    EditFormContext.props.changeEditStatus(initExample !== d.text);
  }
    ...
    return (
      <div>
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          {/* ----------- 返回示例 ------------ */}
          <h2 className="interface-title">返回示例</h2>
          <div className="panel-sub">
            <FormItem className={'interface-edit-item'}>
              <div>
                <AceEditor
                  data={this.state.desc}
                  onChange={this.handleResExample}
                  mode="json"
                  style={{ minHeight: 300 }} />
              </div>
            </FormItem>
